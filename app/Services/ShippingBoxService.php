...
class ShippingBoxService
{
    ...
    public function find(int $id)
    {
        ...
        if (!$shipping_box) {
            ExceptionThrower::throw(new NotFoundException(__('Shipping box not found.', 'kirki-ecommerce'), Response::NOT_FOUND));
        }
    ...
    public function update(UpdateShippingBoxDTO $data)
    {
        ...
        if (empty($shipping_box)) {
            ExceptionThrower::throw(new NotFoundException(__('Shipping box could not be found.', 'kirki-ecommerce'), Response::NOT_FOUND));
        }
        ...
        if (!$data->is_default && $current_default && $current_default->id === $data->id) {
            ExceptionThrower::throw(new NotFoundException(__('At least one default shipping box is required.', 'kirki-ecommerce'), Response::BAD_REQUEST));
        }

        $is_updated = (bool) $shipping_box->update($data->to_array());

        if (!$is_updated) {
            ExceptionThrower::throw(new NotFoundException(__('Shipping box could not be updated.', 'kirki-ecommerce'), Response::NOT_FOUND));
        }
    ...
    public function delete(int $id)
    {
        ...
        if (!$is_deleted) {
            ExceptionThrower::throw(new NotFoundException(__('Shipping box could not be deleted.', 'kirki-ecommerce'), Response::NOT_FOUND));
        }
    ...
    public function bulk_delete(array $ids)
    {
        ...
        if (!$is_deleted) {
            ExceptionThrower::throw(new NotFoundException(__('Shipping boxes could not be deleted.', 'kirki-ecommerce'), Response::NOT_FOUND));
        }
