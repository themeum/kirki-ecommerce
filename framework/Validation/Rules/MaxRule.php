...
class MaxRule extends BaseRule
{
    ...
    {
        if ($this->is_string_value()) {
            return sprintf(__('The %s field must be less than or equal to %s characters.', 'kirki-ecommerce'), $this->last_key_segment(), $this->rule_value);
        } elseif ($this->is_array_value()) {
            return sprintf(__('The %s field must be less than or equal to %s items.', 'kirki-ecommerce'), $this->last_key_segment(), $this->rule_value);
        }

        return sprintf(__('The %s field must be less than or equal %s.', 'kirki-ecommerce'), $this->last_key_segment(), $this->rule_value);
    }
