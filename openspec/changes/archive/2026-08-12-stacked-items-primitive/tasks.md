...
- [ ] 7.3 ~~Confirm `stacked-items-preview.tsx` renders correctly via `tryouts.tsx`.~~ **Obsolete**: `restructure-app-features` deleted both `tryouts.tsx` and `preview-pages/` (the preview/playground harness) as dead code in its group 8. This task's verification path no longer exists; the remaining QA tasks below (7.1, 7.2, 7.4, 7.5) are dev-server checks against real pages and are unaffected — run them against the restructured tree.
