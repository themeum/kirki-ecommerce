...
The system SHALL provide a `TimePicker` that selects an hour and a minute from constrained lists, supporting both 24-hour and 12-hour presentation, while always emitting a 24-hour `HH:mm` string.

#### Scenario: Selecting a time

- **WHEN** the user selects hour `14` and minute `30`
- **THEN** the change handler receives `'14:30'`

#### Scenario: Twelve-hour presentation

- **WHEN** the picker is in 12-hour mode and the user selects `2`, `30`, and the afternoon meridiem
- **THEN** the change handler still receives `'14:30'`

#### Scenario: Minute granularity

- **WHEN** a minute step is supplied
- **THEN** only minutes that are multiples of that step are offered for selection
...
The system SHALL provide form-field wrappers — `DateField`, `DateRangeField`, `TimeField`, and `DateTimeField` — that bind each picker to a named form field and render the same label, description, and error affordances as every other field in the design system.
