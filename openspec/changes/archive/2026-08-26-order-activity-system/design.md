...
### Third correction during implementation
The `OrderActivityResource`/`OrderActivityManager` output was further simplified: `activity_type_label` and `metadata` were dropped from the API response (clients that need the activity type get `activity_type`; nothing external was consuming the raw metadata or a pre-formatted label), and `order_placed()`'s special-cased "draft order" wording (`describe_order_placed()`'s `is_manual` branch, and resolving the actor from `$order->created_by` instead of the ambient user) was removed in favor of the same single `Order placed for %s #%s` wording and the same ambient `resolve_author()` every other activity type already uses — one fewer special case, at the cost of the "Admin created this draft order." copy from the original product screenshots. This is a spec-level simplification (fewer response fields, one less description variant) accepted as a deliberate scope reduction, not a bug fix — flagged here since it changes what "Activity descriptions are human-readable" and the resource shape actually deliver versus the original screenshots discussed in explore mode.

### Pagination
The list endpoints (`GET /orders/{order_id}/activities`, `GET /account/orders/{id}/activities`) now paginate, using the same `ListFilterDTO` + `Pagination::ALL` sentinel pattern already established by `BrandController`/`BrandService` and mirrored across most other list endpoints in this codebase (`OrderController::get()` included) — not a bespoke pagination shape. `OrderActivityService` gained `paginated_for_order(int $order_id, ListFilterDTO $filters)` and `all_for_order(int $order_id)`, both built on a shared protected `list_query()`. `limit`/`page` default the same way every other paginated list here does (`Pagination::LIMIT` = 15, page 1); passing `limit=-1` returns every activity for the order in one response, wrapped in the same `Paginator` envelope (`results`, `total`, `count`, `per_page`, `current_page`, `last_page`, `from`, `to`, `has_more_pages`). No new DTO was introduced — `ListFilterDTO`'s unused fields (`search`, `sort_by`, `sort_order`, `from_date`, `to_date`) are simply ignored, matching how several other simple list endpoints reuse it.
