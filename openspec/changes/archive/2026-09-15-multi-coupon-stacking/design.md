...
**11. Post-hoc naming cleanup: relations named after what they return, DTO uses `coupon_codes`.**
Identified during review: `CalculationContextDTO::$coupons` (Decision 7) reads as a `Coupon[]` collection everywhere else in this codebase (`Cart::coupons()`, `DiscountService::calculate()`'s `$coupons` param) but actually holds `string[]` codes — renamed to `$coupon_codes` to match `CreateOrderPayloadDTO`/`UpdateOrderPayloadDTO`'s already-established naming. Separately, `Cart::coupons()`, `Order::coupons()`, and `OrderItem::coupons()` shared one relation name across three models despite returning three different types (`Coupon[]`, `OrderCoupon[]`, `OrderItemCoupon[]`). The fix is not "name every relation after its own table" (a `cart_coupons()`/`order_coupons()`/`order_item_coupons()` scheme was tried first and reverted) — it's "name every relation after the model it returns," matching this codebase's existing pivot convention (`Coupon::products()`/`categories()`/`customers()` are named after the target model despite going through a pivot table, not `coupon_products()` etc.). `Cart::coupons()` returns `Coupon[]` through the `cart_coupons` pivot, so it keeps the name `coupons()`. `Order::coupons()` and `OrderItem::coupons()` are direct `hasMany` relations whose target model is genuinely backed by the `order_coupons`/`order_item_coupons` tables (not a pivot) — renamed to `order_coupons()`/`order_item_coupons()` so the name matches what's returned, which for a direct hasMany happens to equal the table name. Net effect: the three relations no longer share a name, because they no longer share a name *and* a different return type — `Cart::coupons()` (`Coupon[]`) and `Order::order_coupons()` (`OrderCoupon[]`) are now both distinctly named and distinctly typed. Neither rename touches calculation logic, schema, or API response shapes. The still-open `coupon_code`/`coupon_codes` TODOs are resolved by removing the fallback normalization entirely and updating request validation to accept `coupon_codes` only — `CartUpdateRequest`'s `coupon_code` field is deleted outright rather than renamed, since it was already dead (`UpdateCartAction` never read it; cart coupons apply through the dedicated `apply_coupon`/`remove_coupon` endpoints). See tasks.md §8.
...
- **Unbounded stacking on one item** (two item-scoped coupons both eligible on the same item, applied unconditionally per Non-Goals) could discount a single item heavily.
  → **Correction during implementation**: capping each coupon individually at the item's subtotal turned out not to be enough — two coupons each capped at 100% of an item still sum to 200% "discount" on that item, which would have broken the "sum of coupons = order total" reconciliation invariant this whole change exists to guarantee. `DiscountService::clamp_item_discounts()` was added as a final pass in `calculate()`: when a summed item discount exceeds that item's subtotal, coupons are honored in application order — whichever coupon was applied first keeps its full amount on that item, later coupons only get whatever room is left, and once the item's subtotal is fully covered, any further coupon's contribution to that item is dropped entirely. This is first-come-first-served, not the largest-remainder proportional split used for cart-wide allocation (an earlier draft of this note described it as proportional; the shipped method and its docblock use FCFS instead). Every coupon's reported total still reconciles exactly with what was actually applied. This replaces the "uncapped combined total" statement from the original design — the combined total is now capped, per item, exactly at that item's subtotal.
...
- **Removing the `coupon_code` fallback breaks any client still sending the old singular field** (order-calculation, order-create, order-update) → Mitigation: acceptable under the same pre-production/alpha justification as every other breaking change in this document (Decision 3, Risk above); the frontend follow-up list in tasks.md carries this as an explicit required change.
