...
While the input is empty, option rows SHALL be ordered depth-first and indented in proportion to their depth, so the parent/child structure is legible. While a query is present, rows MUST instead render unindented, each prefixed with its full ancestor path, so a nested match reads in context without its ancestors having to be on screen.
