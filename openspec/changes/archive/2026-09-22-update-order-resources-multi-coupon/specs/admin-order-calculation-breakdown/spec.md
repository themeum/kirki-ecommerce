...
### Requirement: The calculation payload exposes both base and display-currency amounts

The manual order calculation payload SHALL expose, for every monetary
figure, both the store's base-currency amount and the amount converted to
the calculation's target display currency, each as a money object. It
SHALL NOT expose a bare scalar amount key alongside a money object for the
same figure.

#### Scenario: Manual order created in a non-base currency

- **WHEN** a merchant prices a manual order in a currency other than the
  store's base currency
- **THEN** every monetary figure in the payload carries both its
  base-currency money object and its display-currency money object
...
### Requirement: Available shipping methods expose only money objects for their cost

Each available shipping method in the calculation payload SHALL expose its
cost as base and display-currency money objects only, with no bare scalar
cost key.

#### Scenario: Shipping method cost rendered

- **WHEN** the calculation payload lists an available shipping method
- **THEN** that method's cost appears only as its base and display money
  objects, with no separate bare numeric cost key
