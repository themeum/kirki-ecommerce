...
- **Hex pattern without a comma.** The framework's rule parser splits a rule's arguments on commas, so `regex:/^#(?:[0-9a-fA-F]{3}){1,2}$/` reached `preg_match` cut at `{1,`, which rejected every color. That included a renamed Color preset, whose values carry hex codes. The pattern is now `/^#[0-9a-fA-F]{3}(?:[0-9a-fA-F]{3})?$/`, which has no comma or pipe.
- **Focus and drag polish.** A card opened from a preset or from the `+ Add` list focuses the value input, since the name is already filled in. A brand-new card still focuses the name. `MultiSelect` gains a `focusOnMount` prop for this, because lint bans `autoFocus`. Dragging a card now renders a raised `DragOverlay` copy and leaves the slot faded. The overlay is portalled into the app root, because `AnimatedPage` keeps `transform: translateY(0)` from its enter animation. That transform made the fixed overlay measure from the page wrapper, so a click reordered cards and drags were clamped. The `+ Add` search row is 32px high.
