...
- 5 services (`currency`, `payment`, `settings`, `shipping`, `tax`) intentionally have no response validation yet — giving them real catalog schemas is scoped as a separate follow-up change, not bundled into form-schema work.

## Known pre-existing issues (not caused by this change, left as-is / flagged separately)

- `services/payment.ts`'s `getPaymentGateways()` casts the REST response to `PaymentGateway[]` with no validation; the real endpoint returns an object keyed by gateway id, not an array, which crashes `PaymentGatewayComponent`. Flagged as a standalone fix, not bundled into schema-conversion work.
