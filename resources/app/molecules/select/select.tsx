...
type SelectValueOrArray = SelectValue | SelectValue[];
...
type SelectProps = StyleProps &
  LabelFieldProps & {
    value?: SelectValueOrArray;
    defaultValue?: SelectValueOrArray;
    placeholder?: string;
    optionsArray?: SelectOptionItem[];
    onChange?(value: SelectValueOrArray): void;
    onClose?: () => void;
    size?: SelectSize;
    type?: SelectType;
    state?: SelectState;
...
const normalizeValue = (v: SelectValueOrArray | null | undefined): SelectValue[] =>
...
const Select = (props: SelectProps) => {
  ...
  const selectVariants: {
    ...
    size: {
      small: `${CLASS_PREFIX}-select-small`,
      large: `${CLASS_PREFIX}-select-large`,
    },
    type: {
  ...
  const allClassNames = classNames(
    ...
    size ? selectVariants.size[size] : undefined,
    type ? selectVariants.type[type] : undefined,
    state ? selectVariants.state[state] : undefined,
    ...
    if (selectedValues.length > 1) {
      ...
      setSelectedTitle(
        `${selectedOption?.title} +${selectedValues?.length - 1} ${
          extraCount > 1
            ? __('others', 'kirki-ecommerce')
            : __('other', 'kirki-ecommerce')
        }`,
      );
    } else {
      if (selectedValues[0] === ' ' && forceText) {
        setSelectedTitle(forceText);
      } else {
        const selectedOption = optionsArray?.find(
          (item) => !item?.heading && item?.value == selectedValues[0],
        );
  ...
  const handleOptionClick = (e: MouseEvent, option: SelectOptionItem) => {
    if (multiple) {
  ...
  const handleMultipleSelect = (_e: MouseEvent, option: SelectOptionItem) => {
        ...
        >
          ...
          {onNewItemAdd && (
            <>
              ...
              {(optionsArray?.length ?? 0) > 0 && <Separator />}
            </>
          )}
          {optionsArray &&
