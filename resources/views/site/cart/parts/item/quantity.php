...
<div class="kecom-cart-item-quantity">
    <div class="kecom-quantity kecom-quantity-sm">
        ...
        <input class="kecom-quantity-input" type="number" :value="quantity" @input.debounce.200ms="setValue($el.value)" min="1" max="<?php echo esc_html($max_quantity); ?>" aria-label="Quantity">
        <button class="kecom-quantity-btn" type="button" aria-label="Increase" @click.debounce.200ms="increment" <?php disabled($at_max); ?> :disabled="isMax"> <?php Icon::render('plus'); ?></button>
    </div>
