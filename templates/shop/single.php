...
<div class="kirki-ecom-page-wrapper">
    <div class="kirki-ecom-product-wrapper">
        <div class="kirki-ecom-product-image">
            <div class="kirki-ecom-product-image-main">
            ...
            <?php if (count($media)): ?>
                <div class="kirki-ecom-product-image-slider">
                    <?php foreach ($media as $media_item): ?>
                        <div class="kirki-ecom-product-image-thumbnail">
        ...
        <div class="kirki-ecom-product-info">
            <?php if (! empty($ribbon)): ?>
                <div class="kirki-ecom-product-ribbon"><span class="kirki-ecom-product-ribbon-item"><?php echo esc_html($ribbon); ?></span></div>
            <?php endif; ?>
            <h2 class="kirki-ecom-product-title"><?php echo esc_html($product['title']); ?></h1>
                <?php if ($sale_price): ?>
                    <p class="kirki-ecom-product-price">
                        <span><?php echo esc_html($sale_price); ?></span>
                        <del><?php echo esc_html($price); ?></del>
                    </p>
                <?php else: ?>
                    <p class="kirki-ecom-product-price"><?php echo esc_html($price); ?></p>
                <?php endif; ?>
                <hr />
                <?php if (! empty($product['description'])): ?>
                    <?php //@TODO: need a new column called 'short_description' 
                    ?>
                    <div class="kirki-ecom-product-description"><?php echo esc_html($product['description']); ?></div>
                <?php endif; ?>
                <form id="kirki-single-product-form" method="get" style="display: flex; flex-direction: column; gap: 16px; align-items: stretch; width:100%;">
                    <?php if (! empty($attributes)): ?>
                        ...
                        ?>
                            <div class="kirki-ecom-product-attribute">
                                <b class="kirki-ecom-product-attribute-title"><?php echo esc_html($title); ?></b>
                                <div class="kirki-ecom-product-attribute-values">
                    ...
                    <?php endif; ?>
                    <?php if ($quantity > 0): ?>
                        <div class="kirki-ecom-product-quantity-wrapper">
                            <label class="kirki-ecom-product-stock" for="quantity"><?php echo esc_html__('Quantity:', 'kirki-ecommerce'); ?></label>
                            <input class="kirki-ecom-product-quantity" type="number" id="quantity" name="quantity" value="1" min="1" max="<?php echo esc_attr($quantity); ?>">
                            <span class="kirki-ecom-product-stock-label"><?php printf(_n('%d pcs left', '%d pcs left', $quantity, 'kirki-ecommerce'), $quantity); ?></span>
                        </div>
                    <?php endif; ?>
                    <button class="kirki-ecom-add-to-cart" type="button"><?php echo esc_html__('Add to Cart', 'kirki-ecommerce'); ?></button>
                </form>
        </div>
    </div>
    <div class="kirki-ecom-product-additional-info-wrapper">
        <div>
